#!/usr/bin/env python3
"""Enchanter Inference Engine — evidence accumulation over an artifact stream.

Port of ``wixie/shared/scripts/inference-engine.py``.  Algorithms U1-U6
preserved verbatim; paths and env-var names adapted for enchanter-agent.
...
def env_enabled() -> bool:
    ...
    return os.environ.get("ENCHANTER_INFERENCE_ENABLED", "0") == "1"
...
def _state_dir() -> Path:
    """Return state dir, honouring ENCHANTER_INFERENCE_STATE override at call time."""
...
def render_briefing(plugin: str, state_dir: Path | None = None) -> Path:
    ...
    lines = [
        ...
        "",
        "*This briefing is machine-generated by `enchanter/inference/engine.py`.*  ",
        "*Source of truth is `enchanter/state/inference/catalog.json`.*",
        "",
        "---",
        "",
    ]
...
def cmd_emit(args: list[str]) -> int:
    if not env_enabled():
        sys.stderr.write(
            "[inference-engine] ENCHANTER_INFERENCE_ENABLED!=1 — emit skipped\n"
        )
        return 0
    if not args:
