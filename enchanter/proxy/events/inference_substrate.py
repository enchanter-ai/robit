"""enchanter.proxy.events.inference_substrate — Wave 13.2 Agent F.

Bridges the proxy lifecycle to the cross-session learning substrate
(``enchanter.inference.engine``).  Two phases:
...
:mod:`enchanter.inference.engine`.  We never spawn the
...
from enchanter.inference import engine as _inference_engine
...
def _gate_enabled() -> bool:
    ...
    Delegates to :func:`enchanter.inference.engine.env_enabled` so the
