"""enchanter.proxy.events.cost_ledger — Wave 13.1 cost-ledger emitter.
...
The :mod:`enchanter.engines.cost_ledger` engine consumes the builtin
...
#. The engine emits with ``source="cost-ledger"`` and topic
   ...
   :attr:`enchanter.proxy.pipeline._BusRecorder._INTERESTING_SOURCES`
...
class CostLedgerEmitter:
    ...
    :func:`enchanter.proxy.events.load_emitters`.  Module name sorts
    alphabetically AFTER ``builtin`` so the builtin's
    ``mcp.tool.result.received`` publish runs first — that's the event the
    :class:`~enchanter.engines.cost_ledger.adapter.CostLedger` engine
