"""enchanter.proxy.events.rate_limiter — pre-dispatch emitter for the
rate-limiter engine.

Wave 13.1 / Agent A.  Wires the existing
:class:`enchanter.engines.rate_limiter.adapter.RateLimiter` engine into the
...
:meth:`enchanter.core.lifecycle.Orchestrator._wire_plugin`), the first event
...
class RateLimiterEmitter:
    ...
    dedup interaction with :class:`~enchanter.proxy.events.builtin.BuiltinEmitter`.
