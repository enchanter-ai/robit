"""enchanter.proxy.server — stdlib asyncio HTTP/1.1 frontend for the proxy.
...
:mod:`enchanter.mcp_server.http`), routes the request to the matching
provider adapter, runs the canonical request through
:mod:`enchanter.proxy.pipeline`, and writes the rendered response back.
...
class ProxyServer:
    ...
    async def _dispatch(
        ...
        try:
            ...
            logger.exception("ProxyServer: enchanter.proxy.pipeline unavailable")
