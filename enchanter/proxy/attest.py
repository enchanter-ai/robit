...
Sibling module :mod:`enchanter.proxy.pipeline` calls :func:`attest_decision`
...
Configuration (env):
    ...
    ENCHANTER_ATTEST_ENABLED=1     opt-in gate; unset -> attest_decision no-ops
    MIMIR_ISSUER_URL               issuer base URL (default http://localhost:8080)
    ENCHANTER_ATTEST_TOOL_ID       envelope tool_id (default
                                   did:web:enchanter-labs.dev:robit:proxy-gate)
    ENCHANTER_ATTEST_TIMEOUT_S     issuer POST timeout, seconds (default 3)
...
_DEFAULT_TOOL_ID = "did:web:enchanter-labs.dev:robit:proxy-gate"
...
def is_enabled() -> bool:
    ...
    return os.environ.get("ENCHANTER_ATTEST_ENABLED") == "1"
...
def _tool_id() -> str:
    return os.environ.get("ENCHANTER_ATTEST_TOOL_ID", _DEFAULT_TOOL_ID)


def _timeout_s() -> float:
    raw = os.environ.get("ENCHANTER_ATTEST_TIMEOUT_S")
    if raw:
        try:
...
def _platform_default_dir() -> Path:
    if sys.platform.startswith("win"):
        ...
        if appdata:
            return Path(appdata) / "enchanter" / "attest"
        return Path.home() / "AppData" / "Roaming" / "enchanter" / "attest"
    return Path.home() / ".enchanter" / "attest"
...
def _fallback_dir() -> Path:
    return Path(tempfile.gettempdir()) / "enchanter-attest"
