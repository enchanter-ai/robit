"""enchanter.proxy.adapters.anthropic — Anthropic Messages API wire adapter.

Translates between the Anthropic ``/v1/messages`` JSON wire format and the
provider-neutral :mod:`enchanter.proxy.canonical` dataclasses.
...
:func:`enchanter.proxy.upstream.call_upstream` (or ``stream_upstream``) and
