"""enchanter.llm.anthropic_client — AnthropicClient wrapping the Anthropic SDK.

The ``anthropic`` import is deferred to ``__init__`` so that importing
``enchanter.llm`` (e.g., to get ``MockLlmClient``) never fails when the
...
class AnthropicClient:
    ...
    * **OAuth bearer token** (``Authorization: Bearer …``) — Claude.ai
    ...
    def __init__(
        ...
        # Deferred import — so `from enchanter.llm import MockLlmClient` works
