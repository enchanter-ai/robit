"""enchanter.agent.mcp — bridge MCP servers into the agent tool registry.

Wave 15.3 / Agent J. Wires the existing MCP client transport
(``enchanter.transport.stdio.StdioTransport``) up as a source of tools the
...
identical to a local :class:`~enchanter.agent.tools.Tool` to the agent loop.
...
async def register_mcp_tools(
    ...
    ----------
    registry:
        A :class:`~enchanter.agent.tools.ToolRegistry` instance. New tools are
