"""enchanter.agent.app — Textual REPL shell.
...
:class:`~enchanter.agent.widgets.repl.ReplWidget` owns the visual surface;
the :class:`~enchanter.agent.widgets.footer.FooterWidget` owns status +
cost ticker mount.

Imports are still deferred — Textual is heavy and we don't want
``import enchanter.agent`` to drag it in. Call :func:`launch` (or
...
def _build_app_class():
    ...
    Kept in a function so ``import enchanter.agent`` doesn't require Textual.
