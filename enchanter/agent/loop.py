"""enchanter.agent.loop — the user-prompt → LLM → tool loop.

Wave 15.0 ships the contracts + a fully-working mock LLM path. Wave 15.1+ will
swap the mock for real :func:`enchanter.proxy.pipeline.run` calls (the seam is
...
class AgentLoop:
    ...
    Each LLM call goes through :func:`enchanter.proxy.pipeline.run` by
