"""enchanter.agent.cli — `enchanter` binary entry point.

Three modes:

* ``enchanter``                  — Textual REPL (default).
* ``enchanter --version``        — print version, exit 0.
* ``enchanter --session <id>``   — resume a saved session in the REPL.
* ``enchanter "task description"`` — one-shot mode: drive a single turn,
...
async def mock_dispatch(req) -> Union[PipelineResult, VetoResult]:
    ...
    * After a tool_result has come back, respond with a final summary and
    ...
    for msg in reversed(req.messages):
        if msg.role != "user":
            continue
        from enchanter.proxy.canonical import ToolResultPart
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.version:
        _write(f"enchanter {enchanter.__version__}\n")
        return 0
