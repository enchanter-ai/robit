"""enchanter.agent.tools._types — tool contract dataclasses.

Foundation contracts for Wave 15.0. Wave 15.1+ tool implementations conform
to :class:`Tool` (defined in ``enchanter.agent.tools.__init__``); the supporting
...
class ToolResult:
    ...
    is_error:
        ...
        :class:`~enchanter.proxy.canonical.ToolResultPart` so the LLM can
