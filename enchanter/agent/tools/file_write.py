"""enchanter.agent.tools.file_write — create or replace a file.

Destructive by design: this tool always replaces the target file's contents
(no diff, no merge). Use :mod:`enchanter.agent.tools.file_edit` for surgical
changes to an existing file.

Safety contracts:

* Paths resolve via :func:`enchanter.agent.tools._paths.safe_resolve`; writes
