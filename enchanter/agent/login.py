"""enchanter.agent.login — `enchanter login` / `logout` command implementations.

Wave 17.1 wires the existing PKCE OAuth helpers (Wave 16.2) into reachable
CLI commands. The PKCE flow itself lives in
``enchanter.llm._chatgpt_auth.run_pkce_flow`` — this module is the user
interface around it.
...
async def login_chatgpt() -> int:
    ...
    try:
        ...
        _err("retry with: enchanter login chatgpt")
        return 2
...
For ChatGPT subscription: `enchanter login chatgpt`
"""
...
def run_login(args) -> int:
    """Dispatch the ``enchanter login`` subcommand."""
    if args.list:
        return login_list()
    ...
    _err("error: `enchanter login` requires a provider or --list")
    _err("usage: enchanter login {chatgpt,anthropic} | enchanter login --list")
    return 1


def run_logout(args) -> int:
    """Dispatch the ``enchanter logout`` subcommand."""
    if args.all:
        return logout(all_providers=True)
    if args.provider:
        return logout(args.provider)
    _err("error: `enchanter logout` requires a provider or --all")
    return 1
