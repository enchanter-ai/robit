...
frontmatter, and returns one :class:`~robit.conduct.types.ConductRule`
...
from robit.conduct.types import ConductFrontmatterError, ConductRule, EnforcementMode
...
def load_conduct(root: Path | None = None) -> list[ConductRule]:
    ...
    Args:
        root: Root directory of an ``vis``-layout tree.
              Defaults to :data:`~robit.conduct._paths.DEFAULT_VIS_ROOT`.
              The function searches ``<root>/packages/*/conduct/*.md``.

    Returns:
        A list of :class:`~robit.conduct.types.ConductRule` objects,
        one per ``.md`` file found.  The list is sorted by ``(package, name)``
        for deterministic ordering.

    Raises:
        :class:`~robit.conduct.types.ConductFrontmatterError`: propagated
            from :func:`~robit.conduct.frontmatter.parse_frontmatter` when
