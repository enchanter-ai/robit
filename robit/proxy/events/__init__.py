"""robit.proxy.events — pluggable bus-event emitters for the proxy pipeline.

Replaces the hard-coded ``bus.publish(...)`` calls that used to live inline
in :mod:`robit.proxy.pipeline`.  Adding a new engine wire-in is now a
...
:func:`robit.proxy.pipeline._run_emitters` for the swallow site.
"""
