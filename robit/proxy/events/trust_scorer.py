"""robit.proxy.events.trust_scorer — wire proxy completions into the
trust-scorer Beta-Bernoulli posterior.

The trust-scorer engine (``robit.engines.trust_scorer``) maintains a
...
from robit.engines.trust_scorer.adapter import adapter as _trust_engine
...
# the ``_BusRecorder`` in :mod:`robit.proxy.pipeline` (which lists
