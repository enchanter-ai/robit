"""robit.proxy.events.tool_poisoning_scan — pre-dispatch tool-schema scanner wire-in.
...
scan`` engine (``robit.engines.tool_poisoning_scan.adapter``) subscribes
...
:class:`~robit.proxy.pipeline.VetoResult` from :func:`pipeline.run`.

Phase mapping
-------------
The engine declares ``phases=("post-response",)`` and the orchestrator's
wiring (`robit.core.lifecycle._wire_plugin`) short-circuits any inbound
    ...
    async def emit(self, phase: str, ctx: EmitContext) -> None:
        ...
        for tool in tools:
            event = build_event(
                ...
                # check in ``robit.core.lifecycle._wire_plugin``.
                phase="post-response",
