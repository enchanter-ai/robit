...
Entry point declared in pyproject.toml::
    ...
    [project.scripts]
    insighter = "robit.insighter:main"

The ``robit`` binary name is reserved for the 0.5.0 coding-agent CLI
...
import robit
from robit.insighter.format import (
...
def cmd_engines_list(args: argparse.Namespace) -> int:
    ...
    try:
        from robit.loader.discovery import _default_root, find_engine_manifests
        from robit.loader.manifest import parse_manifest
...
def cmd_inference_status(args: argparse.Namespace) -> int:
    try:
        from robit.inference.engine import status
        s = status()
    except Exception as exc:  # noqa: BLE001
...
def cmd_inference_reconcile(args: argparse.Namespace) -> int:
    try:
        from robit.inference.engine import reconcile
        cat = reconcile()
    except Exception as exc:  # noqa: BLE001
...
def cmd_tier_route(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        from robit.runtime.tier_router import TierRouter, UnknownTaskClassError
...
def cmd_serve(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "proxy", None):
        ...
        try:
            from robit.proxy import serve_proxy
        except Exception as exc:  # noqa: BLE001
            _err(f"error: robit.proxy import failed: {exc}")
            return 2
    ...
    try:
        from robit.mcp_server import MCPServer, serve_http, serve_stdio
    except Exception as exc:  # noqa: BLE001
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    try:
        from robit.runtime.models_registry import ModelsRegistry
        from robit.runtime.tier_router import TierRouter, _VALID_TASK_CLASSES
