"""robit.agent.tools.grep — search file contents by regex.

Companion to :mod:`robit.agent.tools.glob` — once the LLM has found files
...
* The optional ``path`` arg resolves via
  :func:`robit.agent.tools._paths.safe_resolve` — searches never escape
  ``ctx.cwd``.
