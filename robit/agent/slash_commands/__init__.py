"""robit.agent.slash_commands — opt-in slash command bundles.

This namespace is reserved for slash-command groupings that ship beyond the
Wave 15.0 built-ins in :mod:`robit.agent.slash`.  Each submodule provides
an ``all_*_commands()`` factory returning a list of objects conforming to the
:class:`robit.agent.slash.SlashCommand` Protocol.

The current bundles:

  * :mod:`robit.agent.slash_commands.plan` — ``/plan``, ``/edit``,
