"""robit.agent.widgets.diff — unified-diff renderer for the REPL.

Consumes the compact diff format emitted by
:mod:`robit.agent.tools.file_edit`:
...
:mod:`robit.agent.tools.file_write`, which emits ``"wrote N bytes to X"``
rather than a diff).
...
class ApprovalPrompt(Container):
    ...
    class Approved(Message):
        ...
        :class:`robit.agent.loop.ApprovalRequested` event; the REPL
