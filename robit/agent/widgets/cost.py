"""robit.agent.widgets.cost — Wave 15.2/Agent I live cost ticker.
...
:class:`robit.agent.loop.TurnComplete` event so the displayed numbers
...
``robit.proxy.events.cost_ledger._compute_cents`` so the proxy and the
...
class CostTicker(Static):
    ...
    Reads pricing via :func:`robit.proxy.events.cost_ledger._compute_cents`
    ...
    def add_turn(
        ...
        :class:`robit.agent.loop.TurnComplete`. The widget updates
