"""Tests for robit.proxy.attest — Mimir attestation emit hook.
...
from robit.proxy import attest, upstream
from robit.proxy.canonical import CanonicalRequest, Message, TextPart
from robit.proxy.pipeline import PipelineOptions, PipelineResult, VetoResult, run, stream
...
def _redirect_state_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("ROBIT_ATTEST_ENABLED", raising=False)
...
def _enabled(monkeypatch):
    monkeypatch.setenv("ROBIT_ATTEST_ENABLED", "1")
    yield
...
async def test_body_shape_matches_attest_request(_enabled, _issuer_ok):
    ...
    body = _issuer_ok[0]
    assert body["tool_id"] == "did:web:enchanter.dev:robit:proxy-gate"
