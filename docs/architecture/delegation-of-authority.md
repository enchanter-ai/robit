...
[`EnchantedEvent`+`PluginAck`](../../robit/core/events.py) — every
...
`robit/core/verdict.py` typed value carrying `pattern_id`/`severity`
...
[`MAX_RESTARTS=3`/`DEFAULT_TIMEOUT_S=5`](../../robit/loader/runtimes/sidecar.py)
...
[`SidecarAdapter._parse_ack`](../../robit/loader/runtimes/sidecar.py)
...
`robit/engines/*/adapter.py` as "14 engines"; the engines tree at
[robit/engines/](../../robit/engines) actually contains 15
...
[errors.py](../../robit/proxy/adapters/errors.py) defines the
...
1. **`EnchantedEvent` + `PluginAck`** —
   [core/events.py](../../robit/core/events.py). The lingua franca
...
2. **`PluginAdapter` Protocol** —
   [core/plugin.py](../../robit/core/plugin.py). Every engine
...
3. **`CanonicalRequest`/`CanonicalResponse`/`CanonicalChunk`** —
   [proxy/canonical.py](../../robit/proxy/canonical.py). All three
...
4. **`EmitContext` + `EmitPhase` + `EventEmitter`** —
   [proxy/events/_types.py](../../robit/proxy/events/_types.py).
...
5. **`EngineManifest`** —
   [loader/manifest.py](../../robit/loader/manifest.py). The single
   ...
   `robit/registry/trust_pin.py` module already exists. `[cost: 3
   files, additive (new field, default-empty), parallel-safe, dual-run]`

6. **JSON-RPC `Request`/`Response`/`ErrorCode`** —
   [protocol/jsonrpc.py](../../robit/protocol/jsonrpc.py). Shared
...
1. **Framework defaults** —
   [`LIFECYCLE_PHASES`](../../robit/core/context.py),
...
2. **Engine author** — manifest declares `required` (fail-closed vs.
   ...
   :100–105](../../robit/core/lifecycle.py) with no inspect-pre-
   raise hook.
...
5. **Cross-session learning** — the inference substrate
   ([inference/engine.py](../../robit/inference/engine.py)) is
...
should be encoded in a single module (`robit/runtime/precedence.py`)
...
[proxy/adapters/errors.py](../../robit/proxy/adapters/errors.py)
...
[proxy/events/cost_ledger.py#L96](../../robit/proxy/events/cost_ledger.py)
holds `_PRICE_CENTS_PER_M_TOKENS` (15 model prefixes). The
`engines/cost_ledger/` engine
([store.py](../../robit/engines/cost_ledger/store.py),
[adapter.py](../../robit/engines/cost_ledger/adapter.py)) tracks
*token totals* but never prices them; it consumes `tool_call_cost` from
the inbound payload instead. Meanwhile
[runtime/data/models-registry.json](../../robit/runtime/data/models-registry.json)
...
([pipeline.py#L426](../../robit/proxy/pipeline.py)) is best-effort
string slicing of `reason` — a real fault line.
**Consolidated owner: a new `robit/core/verdict.py` module** with
...
([cost_ledger.py#L72](../../robit/proxy/events/cost_ledger.py))
...
**Consolidated owner: a `topics.toml` (or `robit/core/topics.py`)
...
owner: a `robit/core/failure_codes.py` Enum** (re-export of the
...
schizophrenia.**
[pipeline.py#L355](../../robit/proxy/pipeline.py) publishes
...
([lifecycle.py#L75](../../robit/core/lifecycle.py)) iterates
...
(`_stream_body` in
[pipeline.py#L668](../../robit/proxy/pipeline.py)) even
...
[proxy/conduct.py#L45](../../robit/proxy/conduct.py) takes only
...
[tier_router.py#L139](../../robit/runtime/tier_router.py) returns
...
constants in [sidecar.py#L73](../../robit/loader/runtimes/sidecar.py).
...
- `Pipeline` emitter chain ← every emitter module under
  ...
  inspection**
  ([proxy/events/__init__.py](../../robit/proxy/events/__init__.py)).
...
([loader/discovery.py:_import_adapter](../../robit/loader/discovery.py))
...
[`SidecarAdapter`](../../robit/loader/runtimes/sidecar.py).
...
- The current default of **coercing any malformed/timeout/crash
  ...
  *advisory* sidecars it's wrong — an advisory plugin should fail
  ...
  [sidecar.py#L168–192](../../robit/loader/runtimes/sidecar.py)
...
- CLI args — `robit/cli/` is tiny (`format.py` only). Today no
...
The sidecar runtime ([sidecar.py](../../robit/loader/runtimes/sidecar.py))
...
- [`secret_mask`](../../robit/engines/secret_mask/adapter.py) —
...
- [`intent_anchor`](../../robit/engines/intent_anchor/adapter.py)
...
- [`tool_poisoning_scan`](../../robit/engines/tool_poisoning_scan/adapter.py)
...
- [`boundary_segmenter`](../../robit/engines/boundary_segmenter/adapter.py)
...
- [`inference_substrate`](../../robit/engines/inference_substrate/adapter.py)
...
[`deep_research`](../../robit/engines/deep_research/pipeline.py).**
...
tier resolution via
[`tier_router.route("orchestrator"|"executor"|"validator")`](../../robit/runtime/tier_router.py)
...
sibling `.md` files
([prompts/](../../robit/engines/deep_research/prompts)) for
fetcher/triangulator/verifier *and* as inline `_SYSTEM`/`_USER_TMPL`
constants in [phases/decompose.py](../../robit/engines/deep_research/phases/decompose.py)
and [phases/cast.py](../../robit/engines/deep_research/phases/cast.py)
...
- **How is agent-engine cost accounted?** Route every agent-engine
  ...
  [`cost_ledger.py`](../../robit/proxy/events/cost_ledger.py)
...
1. **[secret_mask](../../robit/engines/secret_mask/adapter.py)
...
2. **[SecretSanitizingStream](../../robit/proxy/streaming.py)
...
3. **[wire adapters](../../robit/proxy/adapters) malformed-input
...
4. **[destructive_op_gate](../../robit/engines/destructive_op_gate/adapter.py)
   & [cve_pattern_gate](../../robit/engines/cve_pattern_gate/adapter.py)
...
5. **[SidecarAdapter](../../robit/loader/runtimes/sidecar.py)
...
6. **[conduct.py:apply_conduct_to_request](../../robit/proxy/conduct.py)
...
7. **[pipeline.py:run](../../robit/proxy/pipeline.py#L469)
...
8. **[orchestrator.py:Orchestrator](../../robit/core/lifecycle.py)
...
9. **[conduct/loader.py:load_conduct](../../robit/conduct/loader.py)
...
10. **[tier_router.py:TierRouter](../../robit/runtime/tier_router.py)
...
11. **[discovery.py:load_engine_registry](../../robit/loader/discovery.py)
...
12. **[sidecar.py restart budget decisions](../../robit/loader/runtimes/sidecar.py)
...
13. **[upstream.py:call_upstream](../../robit/proxy/upstream.py)
...
14. **[cost_ledger emitter](../../robit/proxy/events/cost_ledger.py)
    ...
    stay in
    [engines/cost_ledger/store.py](../../robit/engines/cost_ledger/store.py)
    once that engine's `ledger_path` is wired by default rather than
    optionally. `[cost: 2 files, additive, parallel-safe, one-shot]`

15. **[cost_ledger/store.py](../../robit/engines/cost_ledger/store.py)
...
18. **[mcp_server/dispatcher.py](../../robit/mcp_server/dispatcher.py)
...
19. **[EmitContext.scratch](../../robit/proxy/events/_types.py#L96)
...
20. **[DEFAULT_PROXY_RULES](../../robit/proxy/conduct.py#L34)
...
- **Scope:**
  - Agent A: `state/audits/secret-mask.jsonl` JSONL sink, wired from
    [secret_mask/adapter.py](../../robit/engines/secret_mask/adapter.py)
    and [streaming.py:SecretSanitizingStream](../../robit/proxy/streaming.py).
  ...
  - Agent B: `state/audits/wire-parse-errors.jsonl` from the three
    wire adapters
    ([anthropic.py](../../robit/proxy/adapters/anthropic.py),
    [openai.py](../../robit/proxy/adapters/openai.py),
    [gemini.py](../../robit/proxy/adapters/gemini.py)) +
    [server.py](../../robit/proxy/server.py) on the
  ...
  - Agent C: configurable byte-cap at the server layer for inbound
    ...
    finding).
- **Files touched:** `robit/engines/secret_mask/adapter.py`,
  `robit/proxy/streaming.py`, three `proxy/adapters/*.py`,
...
- **Scope:**
  - Agent A: in
    [sidecar.py:_parse_ack](../../robit/loader/runtimes/sidecar.py),
  ...
  - Agent B: hoist `timeout_s` and `max_restarts` into
    [`EngineManifest`](../../robit/loader/manifest.py) with
    clamps `1 ≤ timeout_s ≤ 60` and `0 ≤ max_restarts ≤ 10`; default
    to today's values (no breakage). Section 4 and section 6
    additional finding.
- **Files touched:**
  `robit/loader/runtimes/sidecar.py`,
  `robit/loader/manifest.py`, every sidecar engine.toml that
...
- **Scope:**
  - Introduce `robit/core/verdict.py` with typed `Verdict(plugin,
    phase, reason, pattern_id, pattern_name, severity)`. Migrate
    [`SecurityVetoError`](../../robit/core/lifecycle.py#L31),
    [`PluginAck(status="veto")`](../../robit/core/events.py#L31),
    [`VetoResult`](../../robit/proxy/pipeline.py#L145), HTTP 451
    rendering in
    [`proxy/server.py`](../../robit/proxy/server.py#L511), and
  ...
  - Add `state/audits/vetoes.jsonl` (the section 9 closing-paragraph
    ...
    (17) and (18).
- **Files touched:**
  `robit/core/verdict.py` (new),
  `robit/core/lifecycle.py`,
  `robit/core/events.py`,
  `robit/proxy/pipeline.py`,
  `robit/proxy/server.py`,
  `robit/protocol/jsonrpc.py`,
  `robit/mcp_server/dispatcher.py`, plus tests across the rename.
...
- **Agents:** 3 parallel.
- **Scope:**
  - Agent A: new `robit/runtime/precedence.py` declaring the
    hierarchy (framework < engine-author < operator < end-user < —
    inference advisory only). Every override site
    ([conduct.py](../../robit/proxy/conduct.py),
    [tier_router.py](../../robit/runtime/tier_router.py),
    [pipeline.py](../../robit/proxy/pipeline.py)) imports the
  ...
  - Agent C: tier router fallback chain — `route()` returns
    `tuple[str, ...]`;
    [upstream.py](../../robit/proxy/upstream.py) iterates with
...
- **Files touched:**
  `robit/runtime/precedence.py` (new),
  `robit/proxy/conduct.py`,
  `robit/proxy/pipeline.py`,
  `robit/runtime/tier_router.py`,
  `robit/proxy/upstream.py`.
...
- **Agents:** 1 (end-to-end pilot; pairs with Wave 14.1's manifest
  hoist).
- **Scope:**
  - Extend [`EngineManifest`](../../robit/loader/manifest.py)
  ...
  - Convert
    [`intent_anchor/adapter.py`](../../robit/engines/intent_anchor/adapter.py)
    ...
    `robit/engines/intent_anchor/prompts/drift.md`.
  - Route upstream cost through the existing `cost_ledger` path; emit
    ...
    needs).
- **Files touched:**
  `robit/loader/manifest.py`,
  `robit/proxy/pipeline.py`,
  `robit/engines/intent_anchor/adapter.py`,
  `robit/engines/intent_anchor/engine.toml`,
  `robit/engines/intent_anchor/prompts/drift.md` (new).
...
- **Topic registry (`topics.toml` or `robit/core/topics.py`)**
