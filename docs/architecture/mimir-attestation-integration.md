...
Implementation: [`robit/proxy/attest.py`](../../robit/proxy/attest.py),
wired in [`robit/proxy/pipeline.py`](../../robit/proxy/pipeline.py) —
`pipeline.run()` / `pipeline.stream()`, invoked per agent turn from
[`robit/agent/loop.py`](../../robit/agent/loop.py) and
[`robit/agent/plan.py`](../../robit/agent/plan.py). There is no HTTP proxy
frontend on current `main`; the X-Veto header surface lives in beholder, so
attestation and the veto audit hang off the in-process pipeline, not a
...
```json
{
  "tool_id": "did:web:enchanter.dev:robit:proxy-gate",
  "tool_version": "<robit.__version__>",
  "request": {
...
| Env | Default | Meaning |
|---|---|---|
| `ROBIT_ATTEST_ENABLED` | unset (off) | opt-in gate; when unset, `attest_decision` is a no-op |
| `MIMIR_ISSUER_URL` | `http://localhost:8080` | issuer base URL |
| `ROBIT_ATTEST_TOOL_ID` | `did:web:enchanter.dev:robit:proxy-gate` | envelope `tool_id` |
| `ROBIT_ATTEST_TIMEOUT_S` | `3` | issuer POST timeout |
...
- **No SPRT reader.** Next wave: `robit/inference` gains a
